...
Parse.initialize('randomappid', 'randomjavascriptkey');
Parse.serverURL = `${window.location.origin}/client`;
...
const postCloudCodeData = async () => {
  try {
    const result = await Parse.Cloud.run('hello', {}, { context: { message:  `ventry tests` }});
